...
public class DeeplinkUrl {
	...
	DeeplinkUrl(Uri uri) {
		...
		if (scheme != null && !scheme.isEmpty()) {
			data.put(SCHEME_PROPERTY, scheme);
		}
		...
		if (userInfo != null && !userInfo.isEmpty()) {
			...
			if (!parts[0].isEmpty()) {
				data.put(USER_PROPERTY, parts[0]);
			}

			if (parts.length > 1 && !parts[1].isEmpty()) {
				data.put(PASSWORD_PROPERTY, parts[1]);
			}
		}
		...
		if (host != null && !host.isEmpty()) {
			data.put(HOST_PROPERTY, host);
		}

		// Port
		int port = uri.getPort();
		if (port >= 0) {
			data.put(PORT_PROPERTY, port);
		}

		// Path
		String path = uri.getPath();
		if (path != null && !path.isEmpty()) {
			data.put(PATH_PROPERTY, path);

			// Path extension
			int lastDot = path.lastIndexOf('.');
			int lastSlash = path.lastIndexOf('/');
			if (lastDot > lastSlash && lastDot + 1 < path.length()) {
				data.put(PATH_EXTENSION_PROPERTY, path.substring(lastDot + 1));
			}
			...
			if (pathComponents.length > 0) {
				data.put(PATH_COMPONENTS_PROPERTY, pathComponents);
			}
		}
		...
		if (query != null && !query.isEmpty()) {
			data.put(QUERY_PROPERTY, query);
		}

		// Fragment
		String fragment = uri.getFragment();
		if (fragment != null && !fragment.isEmpty()) {
			data.put(FRAGMENT_PROPERTY, fragment);
		}
	}
